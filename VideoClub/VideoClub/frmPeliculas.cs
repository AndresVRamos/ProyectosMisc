...
namespace VideoClub
{
    public partial class frmPeliculas : Form
    {
        ...
        private void frmPeliculas_Load(object sender, EventArgs e)
        {
            ...
            if (txtId.Text == "" || txtNombre.Text == "" || txtDirector.Text == "" || txtFechaEstreno.Text == "" ||txtClasificacion.Text == "" || txtPrecioUnitario.Text == "")
            {
            ...
            if (cadena == "Data Source=ANDRESV-PC\\SQLEXPRESS, 1433;Initial Catalog=VIDEOCLUBA;Persist Security Info=True;User ID=Admin;Password=admin1;" || cadena == "Data Source=ANDRESV-PC\\SQLEXPRESS, 1433;Initial Catalog=VIDEOCLUBA;Persist Security Info=True;User ID=Manager;Password=manager1;")
            {
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show(InsertarPelicula(txtId.Text.Trim(), txtNombre.Text.Trim(), txtDirector.Text.Trim(), txtFechaEstreno.Text.Trim(), txtClasificacion.Text.Trim(), txtPrecioUnitario.Text.Trim()), "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            dgvPeliculas.DataSource = MostrarDatos();
            lblCantidad.Text = "Cantidad de Peliculas Registradas: " + MostrarCantidadPeliculas().ToString();
            Limpiar();
        }
        ...
        {
            MessageBox.Show(ModificarPelicula(txtId.Text.Trim(), txtNombre.Text.Trim(), txtDirector.Text.Trim(), txtFechaEstreno.Text.Trim(), txtClasificacion.Text.Trim(), txtPrecioUnitario.Text.Trim()));
            dgvPeliculas.DataSource = MostrarDatos();
            Limpiar();
        }

        private string InsertarPelicula(string id_pelicula, string nombre, string director, string fecha_estreno, string clasificacion, string precioU)
        {
            string rpta = "";
            sqlcon.Open();
            try
            {
                SqlCommand sqlcmd = new SqlCommand("INSERT INTO PELICULA VALUES(" + id_pelicula + ", '" + nombre + "', '" + director + "', '" + fecha_estreno + "', '" + clasificacion + "', '', " + precioU + ")", sqlcon);
        ...
        private string ModificarPelicula(string id_pelicula, string nombre, string director, string fecha_estreno, string clasificacion, string precio)
        {
            string rpta = "";
            try
            {
                sqlcon.Open();
                SqlCommand sqlcmd = new SqlCommand("UPDATE PELICULA set nombre = '" + nombre + "', director = '" + director + "', fecha_estreno ='" + fecha_estreno + "', clasificacion = '" + clasificacion + "', precio_unitario = " + precio + " WHERE id_pelicula = '" + id_pelicula + "'", sqlcon);
                int filas = sqlcmd.ExecuteNonQuery();
                if (filas != 0)
                {
        ...
        private void dgvPeliculas_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtPrecioUnitario.Text = dgvPeliculas.CurrentRow.Cells["precio_unitario"].Value.ToString();
            txtStock.Text = dgvPeliculas.CurrentRow.Cells["stock"].Value.ToString();
        ...
        private void frmPeliculas_KeyUp(object sender, KeyEventArgs e)
        {
            
        }

        private void frmPeliculas_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void txtFechaEstreno_TextChanged(object sender, EventArgs e)
        {
            if (txtId.Text == "" || txtNombre.Text == "" || txtDirector.Text == "" || txtFechaEstreno.Text == "" || txtClasificacion.Text == "" || txtPrecioUnitario.Text == "")
            {
                btnAgregar.Enabled = false;
        ...
        private void frmPeliculas_MouseClick(object sender, MouseEventArgs e)
        {
            Limpiar();
        }
